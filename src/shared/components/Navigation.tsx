...
export const Navigation = () => {
  ...
  useEffect(() => {
    const url = Utils.stripLocaleUrl(window.location.pathname);
    setPath(url ? url : "/");
  ...
  const getLinkElement = (item: INavigationItem) => {
    ...
    return !(isAuthenticated || shouldUseMockupDataWhenUnauthenticated) ? (
      <Link
      ...
      <a
        key={item.key} // Key must be specified explicitly to suppress console errors.
        // However, Link component has problems with inadeqauate caching when switching between locales in different routes.
        // Using a-tag for the authenticated state; this is a bit slower, bug it works reliably without caching quirks.
        {...props}
      >
        {body}
      </a>
